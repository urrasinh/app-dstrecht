...
export const ControlsPanel: React.FC<ControlsPanelProps> = ({
    ...
    // Auto-expand when selecting a filter with params, auto-collapse for Normal
    useEffect(() => {
        if (hasParams) setPanelState('full');
        else setPanelState('compact');
    }, [currentFilter, hasParams]);
    ...
    return (
        ...
        >
            ...
            {hasParams && isFull && (
                <div className="flex-1 min-h-0 px-4 pt-2 pb-3 border-t border-slate-800/60 bg-slate-950/40 flex flex-col gap-2.5 animate-in fade-in duration-200">
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex flex-col gap-2.5 flex-1 min-h-0 overflow-y-auto no-scrollbar">
            ...
            {/* Hint when collapsed but filter has params */}
            {hasParams && !isFull && (
                <button
                    onClick={() => setPanelState('full')}
                    className="shrink-0 mx-4 mb-2 py-1.5 text-[10px] text-emerald-400 font-bold uppercase tracking-wider bg-emerald-950/30 hover:bg-emerald-950/50 rounded-lg border border-emerald-900/40 flex items-center justify-center gap-1.5 transition-colors"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 15l7-7 7 7" />
                    </svg>
                    {activeFilterDef.params.length} ajuste{activeFilterDef.params.length !== 1 ? 's' : ''} disponible{activeFilterDef.params.length !== 1 ? 's' : ''}
                </button>
            )}
        </aside>
    );
