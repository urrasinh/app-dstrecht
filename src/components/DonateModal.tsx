...
export const DonateModal: React.FC<DonateModalProps> = ({ isOpen, onClose }) => {
    ...
    const Field: React.FC<{ label: string; value: string; keyName: string }> = ({ label, value, keyName }) => (
        <button
            ...
            className="group w-full text-left bg-slate-800/60 hover:bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 transition-colors flex items-center gap-2"
        >
            ...
            <div className="shrink-0">
                {copiedKey === keyName ? (
                    <div className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">Copiado ✓</div>
                ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-slate-400 group-hover:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    return (
        <div className="fixed inset-0 z-[450] bg-black/75 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
            <div
                className="bg-slate-900 border border-amber-500/40 rounded-2xl max-w-md w-full max-h-[90dvh] flex flex-col shadow-2xl"
                ...
                <div className="overflow-y-auto px-5 pb-5 flex flex-col gap-5">
                    ...
                    <section>
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="mt-2 w-full bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors"
                        >
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <button
                                onClick={copyAllBank}
                                className="text-[9px] font-bold uppercase tracking-wider text-emerald-400 hover:text-emerald-300 px-2 py-1 rounded bg-emerald-950/40 border border-emerald-900/50"
                            >
