...
export const Feed: React.FC = () => {
    ...
    if (loading) return <div className="flex-1 flex items-center justify-center text-crema-400 text-sm">Cargando feed…</div>;
    ...
    return (
        <div className="flex-1 min-h-0 flex flex-col bg-tierra-950 overflow-hidden">
            {/* Toolbar */}
            <div className="shrink-0 px-4 py-2.5 border-b border-tierra-800 flex items-center gap-2 bg-tierra-900/60">
                <div className="text-xs text-crema-400 flex-1">
                ...
                {hasSelection && (
                    <>
                        <button
                            ...
                            disabled={busy}
                            className="text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg bg-ocre-500 hover:bg-ocre-400 text-white disabled:opacity-50 flex items-center gap-1"
                        >
                        ...
                        <button
                            ...
                            disabled={busy}
                            className="text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg bg-burdeo-700 hover:bg-burdeo-600 text-white disabled:opacity-50 flex items-center gap-1"
                        >
                        ...
                        <button onClick={clearSelection} className="text-[11px] text-crema-400 hover:text-white px-2">×</button>
                    </>
                )}
                {!hasSelection && (
                    <button onClick={refresh} className="text-[11px] text-crema-400 hover:text-ocre-400 px-2 py-1.5 rounded-lg">↻</button>
                )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3">
                {records.length === 0 ? (
                    <div className="h-full flex items-center justify-center text-tierra-500 text-sm">No hay imágenes registradas</div>
                ) : (
                        ...
                        {records.map(r => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`relative aspect-square rounded-xl overflow-hidden bg-tierra-800 border-2 cursor-pointer transition-all ${sel ? 'border-ocre-500 scale-95' : 'border-transparent hover:border-tierra-600'}`}
                                >
                                    <img
                                    ...
                                    />
                                    {r.featured && (
                                        <div className="absolute top-1.5 left-1.5 bg-ocre-400 text-white p-1 rounded-md shadow-lg">
                                    ...
                                    {sel && (
                                        <div className="absolute inset-0 bg-ocre-500/30 flex items-center justify-center">
                                            <div className="bg-ocre-500 text-white rounded-full p-1.5 shadow-xl">
                                    ...
                                    <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/90 to-transparent p-2 text-[10px] text-white">
                                        ...
                                        <div className="flex items-center gap-2 text-[9px] text-crema-300 mt-0.5">
                                            {r.lat != null && r.lon != null && (
                                                <a
                                                    ...
                                                    className="text-ocre-400 hover:underline"
                                                >📍 GPS</a>
                                            )}
            ...
            {confirmDelete && (
                <div className="fixed inset-0 z-[400] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => !busy && setConfirmDelete(false)}>
                    <div className="bg-tierra-900 border border-burdeo-900/60 rounded-2xl max-w-sm w-full p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex gap-2 mt-4">
                            <button disabled={busy} onClick={() => setConfirmDelete(false)} className="flex-1 py-2.5 rounded-lg bg-tierra-800 text-crema-300 text-sm font-semibold disabled:opacity-50">Cancelar</button>
                            <button disabled={busy} onClick={onDelete} className="flex-1 py-2.5 rounded-lg bg-burdeo-700 hover:bg-burdeo-600 text-white text-sm font-semibold disabled:opacity-50">{busy ? 'Eliminando…' : 'Eliminar'}</button>
                        </div>
                    </div>
                </div>
            )}
